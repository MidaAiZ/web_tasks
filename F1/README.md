...
###实现函数
  ...
  7.跳转到指定的图片 lunbo("gotoImg")

##插件使用示例说明

###html文档准备：
            我们首先需要创建一个html文档，并构建一个一级父级容器用于存放图片列表和一些其它控件的次级父级容器，
                例如：
            ...
            需注意的是我们必须要为存放图片的<li>标签设置data-role="image-list"属性，插件通过这个属性来获取图片张数
                例如：
            ...
            如果需要添加用来显示图片张数以及图片显隐状态的示数栏，需要在一级父容器内（或其子级容器内）添加一个<ul>标签
            并且为<ul>标签设置data-role="number-container属性"
                 例如：
                     <div class="num-container">    //二级父容器
                           <ul data-role="number-container">  //我们需要自己为该ul容器设置CSS样式，比如长宽位置等，插件并不会设置该父容器的CSS
                                //这里建议不要再添加其它元素 插件会自动在此添加标签
                           </ul>
                     </div>
            值得注意的是我们并不需要在该ul标签内写入任何标签，插件将根据图片张数在ul标签内自动添加li标签并为li标签设置插件默认CSS属性，
            我们可以在插件的setNumStyle中设置li标签的自定义CSS样式，但是ul标签的样式却需要您自己设置，如未设置可能无法正确显示
...
###调用插件函数设置图片轮播
        ...
        6.设置显示轮播图片数目的及显示状态的示数栏
                如果希望用户能知道正在轮播的图片张数以及目前显示的是第几张图片，那么示数栏是必不可少的控件
                lunboPlugin插件支持示数栏的创建，并且内建了默认样式。
                设置示数栏只需要在父级容器中添加以下子容器，并设置data-role="number-container"的属性：

                    <ul data-role="number-container">  //我们需要自己为该ul容器设置CSS样式，如未设置样式示数栏可能无法正确显示
        ...
        7.为示数栏设置自定义样式
                插件内为示数栏内的元素(即<li>标签)设置了默认的CSS样式，默认样式表如下：
                 ...
                 当然，我们也可以单独的传入"defaultsCSS" 样式或"activeCSS" 样式，只改变对应状态下的CSS样式。
                 如果在调用时只传入"setNumStyle"参数  如$("#container").lunbo("setNumStyle") 插件将给示数栏设置默认样式。
                 如果不需要改变示数栏的样式，并不需要调用该方法，其实在初始化参数时样式栏就已经被设置默认样式了。

        8.返回指定节点在同辈中的索引号
                我们还可以获取当前节点在同辈中的索引号。例如：

                $("[data-role='number-container'] li:first-child").lunbo("getIndex")); });

                通过这行代码，我们获取了示数栏底部第一个<li>标签的索引号，即0。 （索引号以0为开始，即第一个元素的索引号为0）
...
##插件缺陷
    1.插件内部使用了大量的全局变量，大部分功能都是依靠全局变量实现的，这也就意味着一个html文档只能使用一次这个插件
    在同一个页面中只能创建一个图片轮播的模块，如果用户初始化了多个轮播模块，插件不会报错，但是只有最后初始化的一个轮播模块能够使用
    这个问题现在暂时无法解决，如果需要解决可能要重新搭建插件的逻辑，工作量比较大。
    2.本插件不支持传入数据的保存（虽然好像有没有什么要保存的数据~）
